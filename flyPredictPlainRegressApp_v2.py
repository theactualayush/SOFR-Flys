...
def fetch_and_calculate_flies(token, instruments, count=200):
    ...
    # Create Flies
    # Ensure instruments are sorted as provided by user (front-to-back)
    available_legs = [inst for inst in instruments if inst in df_pivot.columns]
    
    fly_df = pd.DataFrame(index=df_pivot.index)
    
    # We need at least 3 legs to make a Fly
    for i in range(len(available_legs) - 2):
        leg1 = available_legs[i]
        leg2 = available_legs[i+1]
        leg3 = available_legs[i+2]
...
num_contracts = st.sidebar.number_input("Number of Contracts to Fetch", min_value=3, max_value=40, value=18, step=1)
...
st.title("SOFR Fly Curve Regression App")
...
if api_token and instruments_list:
    ...
    if df is not None and not df.empty:
        ...
        if len(fly_options) == 0:
            st.warning("Not enough consecutive instruments provided to calculate a Fly (need at least 3).")
        else:
            selected_fly = st.selectbox("Select the base fly:", fly_options)

            if selected_fly:
                # df passed to run_regression needs to have dates in the index, which it does.
                # The run_regression function expects 'df' columns to be the fly columns
                # We need to minimally trick run_regression to expect fly names, and drop NA.
                # In the original, the first column was presumed to be date if reset, but here index is Date.
                ...
                if regression_df is not None:
                    ...
                    if st.button("Predict Curve"):
                        ...
                        ax.set_title(f"Live vs Predicted Fly Curve (Base: {selected_fly})")
                        ax.set_ylabel("Price")
